...
class BikeSparsFormPage extends ConsumerStatefulWidget {
  final SellList? productToEdit;

  const BikeSparsFormPage({super.key, this.productToEdit});

  @override
  ConsumerState<BikeSparsFormPage> createState() => _BikeSparsFormPageState();
}

class _BikeSparsFormPageState extends ConsumerState<BikeSparsFormPage> {
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Center(
                  child: Text(
                    isEditing ? "Update Spare Parts Listing" : "Spare Parts Listing",
                  ...
                  child: Column(
                    ...
                    children: [
                      FormBody(
                        ...
                        helper: "Mention the key features (e.g., part type, compatible vehicle, brand)",
                      ),
                      SizedBox(height: 15.h),
                      FormBody(
                        labeltxt: "Description*",
                        controller: descriptionController,
                        helper: "Include condition, compatibility, and reason for selling",
                        maxLength: 4096,
                      ),
                      SizedBox(height: 15.h),
                      FormBody(
                      ...
                      /*  onPressed: isLoading
                          ...
                          try {
                            ...
                            if (e is DioError && e.response?.statusCode == 429) {
                              ...
                              errorMessage = e.response?.data['message'] ??
                                  "Failed to process spare parts listing. Please try again.";
                            }
                            Fluttertoast.showToast(
                              msg: errorMessage,
                              toastLength: Toast.LENGTH_LONG,
                        ...
                        onPressed: isLoading
                          ...
                          // Images ko try ke bahar convert karo (catch mein bhi use hoga)
                          List<MultipartFile> imageFiles = [];
                          for (var img in images) {
                            imageFiles.add(await MultipartFile.fromFile(
                              img.path,
                          ...
                          // Yeh data PlanPage ko bhejne ke liye ready karo
                          ...
                          try {
                            ...
                            if (isEditing && widget.productToEdit != null) {
                              await service.updateProduct(widget.productToEdit!.id!, productData);
                              Fluttertoast.showToast(
                                msg: "Spare Parts Listing Updated Successfully",
                                toastLength: Toast.LENGTH_LONG,
                              );
                            ...
                            if (e is DioError) {
                              ...
                              if (errorMessage.toLowerCase().contains("plan") ||
                                  ...
                                  [402, 403].contains(e.response?.statusCode)) {
                                needsPlan = true;
                              }
                            }
                            ...
                            if (needsPlan) {
                              // PlanPage pe jao aur PENDING PRODUCT DATA pass kar do!
                              Navigator.push(
                                context,
                                CupertinoPageRoute(
                                  builder: (context) => PlanPage(
                                    productData,  // ← Yeh zaroori hai!
                                    true,         // requiresPlan = true
                                  ),
                                ),
                              );
                            }
                            // Else: User form pe reh jaye ya manually Home jaye (no auto navigation on normal error)
                          }
                        },
                        child: Center(
                          child: isLoading
